...
class Insights {
	...
	public function __construct( $client, $name = null, $file = null ) {
		...
		if ( is_object( $client ) && is_a( $client, 'Disco\Libs\WebAppick\AppServices\Client' ) ) {
			$this->client = $client;
		}
	}
	...
	protected function data_we_collect() {
		$data = [
			esc_html__( 'Server environment details (php, mysql, server, WordPress versions).', 'disco' ),
		];
	...
	public function admin_notice() {
        ...
        if ( ! $this->__is_local_server() ) {
			...
			if ( empty( $this->notice ) ) {
				$notice = sprintf(
					apply_filters(
						...
						esc_html__( 'Want to help make %1$s even more awesome? Allow %1$s to collect non-sensitive diagnostic data and usage information.', 'disco' )
					),
			...
			$notice .= ' (<a class="' . $this->client->getSlug() . '-insights-data-we-collect" href="#">' . esc_html__( 'what we collect', 'disco' ) . '</a>)';
			$notice .= '<p class="description" style="display:none;">' . implode( ', ', $this->data_we_collect() ) . '. ' . esc_html__( 'No sensitive data is tracked.', 'disco' ) . '</p>';
			echo '<div class="updated"><p>';
			echo wp_kses_post($notice); // phpcs:ignore xss ok
			echo '</p><p class="submit">';
			echo '&nbsp;<a href="' . esc_url( $this->get_opt_out_url() ) . '" class="disco-notice-secondary-button">' . esc_html__( 'No thanks', 'disco' ) . '</a>';
			echo '&nbsp;<a href="' . esc_url( $this->get_opt_in_url() ) . '" class="disco-notice-primary-button">' . esc_html__( 'Allow', 'disco' ) . '</a>';
			echo '</p></div>';
	...
	private function __get_uninstall_reasons() {
		...
		$reasons = [
			[
				...
				'text'        => esc_html__( 'I couldn\'t understand how to make it work', 'disco' ),
				'type'        => 'textarea',
				'placeholder' => esc_html__( 'Would you like us to assist you?', 'disco' ),
			],
			[
				'id'          => 'found-better-plugin',
				'text'        => esc_html__( 'I found a better plugin', 'disco' ),
				'type'        => 'text',
				'placeholder' => esc_html__( 'Which plugin?', 'disco' ),
			],
			[
				'id'          => 'not-have-that-feature',
				'text'        => esc_html__( 'The plugin is great, but I need specific feature that you don\'t support', 'disco' ),
				'type'        => 'textarea',
				'placeholder' => esc_html__( 'Could you tell us more about that feature?', 'disco' ),
			],
			[
				'id'          => 'is-not-working',
				'text'        => esc_html__( 'The plugin is not working', 'disco' ),
				'type'        => 'textarea',
				'placeholder' => esc_html__( 'Could you tell us a bit more whats not working?', 'disco' ),
			],
			[
				'id'          => 'looking-for-other',
				'text'        => esc_html__( 'It\'s not what I was looking for', 'disco' ),
				'type'        => '',
				'placeholder' => '',
			],
			[
				'id'          => 'did-not-work-as-expected',
				'text'        => esc_html__( 'The plugin didn\'t work as expected', 'disco' ),
				'type'        => 'textarea',
				'placeholder' => esc_html__( 'What did you expect?', 'disco' ),
			],
			[
				'id'          => 'debugging',
				'text'        => esc_html__( 'Temporary deactivation for debugging', 'disco' ),
				'type'        => '',
				'placeholder' => '',
			],
			[
				'id'          => 'other',
				'text'        => esc_html__( 'Other', 'disco' ),
				'type'        => 'textarea',
				'placeholder' => esc_html__( 'Could you tell us a bit more?', 'disco' ),
			],
		];
		...
		if ( empty( $this->ticketTemplate ) || empty( $this->ticketRecipient ) || empty( $this->supportURL ) ) {
			wp_send_json_error(
				sprintf(
					...
					esc_html__( 'Something Went Wrong.', 'disco' ),
					esc_html__( 'Please try again after sometime.', 'disco' )
				)
			);
			wp_die();
		}
			...
			if ( $isSent ) {
				...
				if ( false !== $supportResponse ) {
					...
					$this->supportResponse = sprintf(
						'<h3>%s</h3>',
						esc_html__( 'Thank you -- Support Ticket Submitted.', 'disco' )
					);
				}
				...
				if ( false !== $supportErrorResponse ) {
					...
					$this->supportErrorResponse = sprintf(
						...
						esc_html__( 'Something Went Wrong. Please Try Again After Sometime.', 'disco' )
					);
				}
				wp_send_json_error( $this->supportErrorResponse );
			}
		} else {
			wp_send_json_error( sprintf( '<p class="mui-error">%s</p>', esc_html__( 'Missing Required Fields.', 'disco' ) ) );
		}
		wp_die();
	}
	...
	public function deactivate_scripts() {
		...
		?>
		<div class="wapk-dr-modal" id="<?php echo esc_attr( $this->client->getSlug() ); ?>-wapk-dr-modal" aria-label="<?php /* translators: 1: Plugin Name */ printf( esc_attr__( '&ldquo;%s&rdquo; Uninstall Confirmation', 'disco' ), esc_attr( $this->client->getName() ) ); ?>" role="dialog" aria-modal="true">
			<?php if ( $showSupportTicket ) { ?>
			<div class="disco-deactivation-modal wapk-dr-modal-wrap support" style="display: none;">
				<div class="wapk-dr-modal-header">
					<h3><?php esc_html_e( 'Submit Support Ticket.', 'disco' ); ?></h3>
					<a href="javascript:void 0;" class="wapk-dr-modal-close" aria-label="<?php esc_attr_e( 'Close', 'disco' ); ?>">
				...
				<div class="wapk-dr-modal-body">
					<div class="wapk-row mui col-2 col-left">
						<label for="wapk-support-name" class="<?php echo ! empty( $displayName ) ? 'shrink' : ''; ?>"><?php esc_html_e( 'Name', 'disco' ); ?></label>
					...
					<div class="wapk-row mui col-2 col-right">
						<label for="wapk-support-email" class="shrink"><?php esc_html_e( 'Email', 'disco' ); ?></label>
						<div class="wapk-form-control">
					...
					<div class="clear"></div>
					<div class="wapk-row mui col-2 col-left">
						<label for="wapk-support-subject"><?php esc_html_e( 'Subject', 'disco' ); ?></label>
						<div class="wapk-form-control">
							<input type="text" name="subject" id="wapk-support-subject" required>
						</div>
					</div>
					<div class="wapk-row mui col-2 col-right">
						<label for="wapk-support-website" class="shrink"><?php esc_html_e( 'Website', 'disco' ); ?></label>
						<div class="wapk-form-control">
					...
					<div class="wapk-row mui">
						<label for="wapk-support-message"><?php esc_html_e( 'Message', 'disco' ); ?></label>
						<div class="wapk-form-control">
				...
				<div class="wapk-dr-modal-footer">
					<button class="disco-notice-primary-button send-ticket"><?php esc_html_e( 'Send Message', 'disco' ); ?></button>
					<button class="disco-notice-secondary-button close-ticket"><?php esc_html_e( 'Cancel', 'disco' ); ?></button>
				</div>
			</div>
			<?php } ?>
			<div class="disco-deactivation-modal wapk-dr-modal-wrap reason">
				<div class="wapk-dr-modal-header">
					<h3><?php esc_html_e( 'If you have a moment, please let us know why you are deactivating:', 'disco' ); ?></h3>
					<a href="javascript:void 0;" class="wapk-dr-modal-close" aria-label="<?php esc_attr_e( 'Close', 'disco' ); ?>">
						<!--suppress HtmlUnknownAttribute -->
						<svg class="" focusable="false" viewBox="0 0 24 24" aria-hidden="true" role="presentation">
							<path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"></path>
				...
				<div class="wapk-dr-modal-body">
					...
					<div class="response" style="<?php echo ( $showSupportTicket ) ? 'display: block;' : ''; ?>">
						<div class="disco-deactivation-modal wrapper">
						<?php if ( $showSupportTicket ) { ?>
							<p><?php esc_html_e( 'In trouble? Please submit a support request.', 'disco' ); ?></p>
							<p>
								<a href="#" class="disco-notice-secondary-button not-interested"><?php esc_html_e( 'Not Interested', 'disco' ); ?></a>
								<button class="disco-notice-primary-button open-ticket-form"><?php esc_html_e( 'Open Support Ticket', 'disco' ); ?></button>
							</p>
						<?php } ?>
						</div>
					</div>
				...
				<div class="wapk-dr-modal-footer">
					<a style="background:transparent !important;" href="#" class="button button-link dont-bother-me disabled"><?php esc_html_e( 'I rather wouldn\'t say', 'disco' ); ?></a>
					<button class="disco-notice-secondary-button deactivate"><?php esc_html_e( 'Submit & Deactivate', 'disco' ); ?></button>
					<button class="disco-notice-primary-button modal-close"><?php esc_html_e( 'Cancel', 'disco' ); ?></button>
				</div>
			</div>
		</div>
                    ...
                    function _ajax(data, buttonElem, cb) {
                        ...
                        return $.ajax({
                            ...
                            beforeSend: function () {
                                ...
                                buttonElem.text('<?php esc_html_e( 'Processing...', 'disco' ); ?>');
                            },
